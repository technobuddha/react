...
{
	constructor(rootElement, divElement)
	{
	...
	public		updateBuffer(frequencyData)
	{
		this.min	= -150;
		this.max	= 0;
		...
		{
			let data	= frequencyData[ Math.round(frequencyData.length / this.buffer.length * i)];
			// clamp into range
	...
	public		paintSpectrum()
	{
		let canvasContext	= this.canvasRef.getContext('2d');
		...
		let yFactor	= this.canvasRef.height / this.range;

		for(let i = 0; i < this.buffer.length - 1; ++i)
		{
			canvasContext.moveTo(i + 0.5, this.buffer[i + 0] * -1.0 * yFactor);
			canvasContext.moveTo(i + 1.5, this.buffer[i + 1] * -1.0 * yFactor);
		}
