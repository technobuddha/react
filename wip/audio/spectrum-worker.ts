...
{
	public		toFrequencyDomain(timeDomainRealIn, timeDomainImagIn, start: number, len: number, realOut, imagOut)
	{
		start	= _.defaultTo(start,	0);
		len		= _.defaultTo(len,		timeDomainRealIn.length);

		if(!MathEx.isPowerOfTwo(len)) throw "The length of the timeDomain has to be power of two!";
	...
	public		fromFrequencyDomain(freqDomainRealIn, freqDomainImagIn, realOut, imagOut)
	{
	...
	public		toAmplitudeSpectrum(timeDomainData, sampleRate, start, len, windowSize, windowFuncId)
	{
	...
	public		toAmplitudeSpectrumFromAudioChannel = function toAmplitudeSpectrumFromAudioChannel(audioChannel, start, len, windowSize, windowFuncId)
	{
		return this.toAmplitudeSpectrum(audioChannel.data, audioChannel.sampleRate, start, len, windowSize, windowFuncId);
	};
}