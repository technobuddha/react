...
class ACFIRFilter
{
	...
	public		resultDivFactor:		number;
	public		length:					number;
	public		lengthDiv8:				number;
	public		filterCoeffs:			Float32Array;
	public		resultDivider:			number;

	public	evaluateFilter(dest: number[], src: number[], numSamples: number): number
	{
		let dScaler = 1.0 / this.resultDivider;

		if (this.length === 0) debugger;

		let end		= numSamples - this.length;
		for(let j = 0; j < end; ++j)
		{
			let sum		= 0;
			for(var i = 0; i < this.length; ++i)
			{
				sum		+= src[j + i] * this.filterCoeffs[i];
			}
	...
	public	setCoefficients(coeffs: Float32Array, newLength: number, uResultDivFactor: number): void
	{
		...
		this.resultDivider		= Math.pow(2.0, this.resultDivFactor);

		this.filterCoeffs		= new Float32Array(this.length);
		for(let i = 0; i < this.filterCoeffs.length; ++i)
		{
